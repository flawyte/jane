...
module.exports = {
  ...
  'createColumn': function(assert) {
    ...
    assert.equal('first_name VARCHAR NOT NULL', gen.createColumn(attr));
    attr.nullable = true;
    assert.equal('first_name VARCHAR DEFAULT NULL', gen.createColumn(attr));
    attr.defaultValue = 'foobar';
    assert.equal('first_name VARCHAR DEFAULT "foobar"', gen.createColumn(attr));
  ...
  'createColumnPrimaryKey': function(assert) {
    ...
    assert.equal('id INTEGER PRIMARY KEY AUTOINCREMENT', gen.createColumnPrimaryKey('id'));
  ...
  'createDatabase': function(assert) {
    ...
    assert.equal(null, gen.createDatabase('jane'));
  ...
  'dropDatabase': function(assert) {
    ...
    assert.equal(null, gen.dropDatabase('jane'));
